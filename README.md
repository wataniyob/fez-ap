...
- Edit `UserProperties.xml`
  - Remove the `UserPropertiesNotSetUp` property
  ...
  - **Optional but recommended**: Run the git command `git update-index --skip-worktree UserProperties.xml` so that this file no longer shows up as "changed" in git and you don't need to worry about accidentally committing your changes
